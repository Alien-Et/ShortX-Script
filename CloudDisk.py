...
- 新增去体验AI相机任务。
- 新增五一福利任务。
- deviceId 生成逻辑改为使用数美设备指纹算法（从 smid.py 整合）
...
def generate_shumei_device_id() -> str:
    """生成数美设备ID（smid）"""
    now = time.localtime()
...
def generate_device_id():
    ...
    # 尝试使用数美设备指纹生成逻辑
    try:
        shumei_device_id = generate_shumei_device_id()
        # 将 JSON 字符串进行 base64 编码，保持与原有格式兼容
        return base64.b64encode(
            shumei_device_id.encode('utf-8')).decode('utf-8')
    except Exception as e:
        print(f'数美设备指纹生成失败，使用备用方案: {e}')
        # 备用方案：使用原有的简单生成方式
        device_info = {
...
def get_env_device_id():
    return (os.getenv('ydyp_device_id') or os.getenv('YDYP_DEVICE_ID')
            or '').strip()
...
def get_storage_base_dir():
    ...
    custom_storage_dir = (os.environ.get('ydyp_storage_dir')
...
def print_device_id_notice():
    ...
    cached_accounts = [
        account for account, info in cached_data.items()
        if isinstance(info, dict) and info.get('deviceId')
    ]
    if cached_accounts:
        print(
            f"已发现本地缓存 deviceId {len(cached_accounts)} 个账号；新账号将自动写入 ydyp_device_ids.json，deviceId 默认留空，等待手动填写"
        )
        return
...
class YP:
    ...
    def __init__(self, cookie):
        try:
            ...
            self.auth_token = extract_raw_token(self.Authorization,
                                                self.account)

            if len(self.account) >= 11:
                self.encrypt_account = self.account[:3] + "****" + self.account[
                    -4:]
            else:
            ...
            self.treeHeaders = {
                ...
                'User-Agent': ua,
                'Referer':
                'https://happy.mail.10086.cn/jsp/cn/garden/wap/index.html?sourceid=1003',
                'Cookie': '',
            }
    ...
    def get_storage_record(self):
        ...
        return (self.token_storage.get('accounts') or {}).get(
            self.account, {}) or {}
    ...
    def save_authorization_record(self,
                                  token=None,
                                  user_domain_id=None,
                                  refreshed=False,
                                  expires_at=None,
                                  market_device_id=None,
                                  x_device_info=None,
                                  market_device_generated=None):
        ...
        record_expires_at = expires_at or old_record.get('expiresAt') or (
            current_millis() + TOKEN_VALID_TIME)
        record = {
            **old_record,
        ...
        if user_domain_id or old_record.get('userDomainId'):
            record['userDomainId'] = user_domain_id or old_record.get(
                'userDomainId')
        final_market_device_id = market_device_id or old_record.get(
            'marketDeviceId')
        ...
        if market_device_generated is None:
            if 'marketDeviceGenerated' in old_record:
                record['marketDeviceGenerated'] = old_record.get(
                    'marketDeviceGenerated')
        else:
    ...
    def refresh_authorization_token(self, force=False):
        ...
        refresh_headers = {
            ...
            'User-Agent':
            'Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.47(0x18002f2c) NetType/WIFI Language/zh_CN miniProgram/wx4e4ed37286c816c2',
        ...
        encrypted_data = aes_encrypt({'phoneNumber': self.account},
        ...
        is_success = (code in ('0', '00', '000', '0000')) or bool(success) or (
            code.startswith('0') and len(code) <= 4)
        if not is_success or not isinstance(data, dict):
            self.log(
                f"-Authorization刷新失败: {refresh_data.get('message') or refresh_data.get('msg', '未知错误')} (code={code or 'unknown'})"
            )
            return True
        ...
        self.save_authorization_record(token=self.Authorization,
                                       refreshed=True,
                                       expires_at=expires_at)
        self.log(f'-Authorization自动刷新成功: {reason}')
        self.log(
            f"-Authorization有效期: {datetime.fromtimestamp(expires_at / 1000).strftime('%Y-%m-%d %H:%M:%S')}"
        )
        return True
    ...
    def sync_token_storage(self):
        ...
        save_token_info(self.account, self.Authorization, expires_at,
                        last_refresh_at)
    ...
    def seed_market_device_cookie(self):
        ...
        cookie_value = device_id[
            1:] if self.market_device_from_env and device_id.startswith(
                'B') else device_id
        if any(
                cookie.name.startswith('.thumbcache_')
                and unquote(cookie.value) == cookie_value
                for cookie in self.session.cookies):
            return
        self.session.cookies.set(f'.thumbcache_{self.account}',
                                 cookie_value,
                                 domain='m.mcloud.139.com',
                                 path='/')

    def build_market_headers(self, extra_headers=None, referer=None):
        headers = dict(self.market_headers)
        headers['Referer'] = referer or headers.get(
            'Referer') or self.build_market_page_url()
    ...
    def prepare_signin_center_session(self, for_receive=False, source_id=None):
        ...
        self.send_request(page_url,
                          headers=self.build_market_headers(referer=page_url),
                          cookies=self.market_cookies,
                          retries=1)
        for keyword in (
                'newsignin_index_pv',
        ...
        if for_receive:
            self.post_signin_journaling('newsignin_index_receive_type',
                                        current_source_id)
        return True

    def click_task(self, task_id, key='task'):
        return self.request_market_json(
            f'{self.market_base_url}/market/signin/task/click?key={key}&id={task_id}'
        )

    def get_notice_status(self):
        send_data = self.request_json(
    ...
    def build_cloud_file_headers(self):
        ...
        return {
            ...
            'x-yun-api-version': 'v1',
            'x-yun-client-info':
            '6|127.0.0.1|1|12.1.0|realme|RMX5060|BCFF2BBA6881DD8E4971803C63DDB5E4|02-00-00-00-00-00|android 15|1264X2592|zh||||032|0|',
    ...
    def build_share_headers(self):
        ...
        return {
            ...
            'x-yun-app-channel': '10000023',
            'x-yun-client-info':
            f'||9|{self.client_version}|Chrome|143.0.7499.146|codextestshare||Windows 10||zh-CN|||Q2hyb21l||',
            'x-yun-module-type': '100',
            'x-yun-svc-type': '1',
            'x-SvcType': '1',
    ...
    def is_cleanup_upload_file(item):
        if item.get('type') != 'file' or item.get('parentFileId') != '/':
            return False
        name = item.get('name', '')
        if not (name.endswith('.txt') and (name.startswith('auto_upload_')
                                           or name.startswith('auto_share_'))):
    ...
    def complete_share_file_task(self, task):
        ...
        if response:
            success = (response.get('success')
                       and result.get('resultCode') == '0') or (str(
                           response.get('code')) == '0' and bool(outlinks))
        if not success:
            if response:
                msg = result.get('resultDesc') or response.get(
                    'message') or response.get('msg', '未知错误')
            else:
                msg = '接口无响应'
    ...
    def build_ai_headers(self, use_client_info=False):
        ...
        headers = {
            ...
            'sec-ch-ua':
            '"Not A(Brand";v="8", "Chromium";v="130", "Mobile Safari";v="130"',
            'sec-ch-ua-mobile': '?1',
            'X-Requested-With': 'com.chinamobile.mcloud',
        ...
        if use_client_info:
            ...
            headers[
                'x-yun-client-info'] = f'4||1|{self.client_version}|Apple|iPhone 16 Pro|{device_id.lstrip("B")}|iOS 18.7|||||'
            headers['x-yun-app-channel'] = '101'
            return headers
        headers['Accept'] = '*/*'
        headers[
            'x-DeviceInfo'] = f'||36|{self.client_version}|Apple|iPhone 16 Pro|{device_id.lstrip("B")}|iOS 18.7|||||'
        return headers

    def encrypt_ai_tool_account(self):
        if AES is None or pad is None:
            raise ImportError('未安装 pycryptodome，无法加密AI工具账号')
        cipher = AES.new(AI_TOOL_ACCOUNT_AES_KEY.encode('utf-8'), AES.MODE_CBC,
                         AI_TOOL_ACCOUNT_AES_IV.encode('utf-8'))
        encrypted = cipher.encrypt(
            pad(self.account.encode('utf-8'), AES.block_size))
        raw = AI_TOOL_ACCOUNT_AES_IV + base64.b64encode(encrypted).decode(
            'utf-8')
        return base64.b64encode(raw.encode('utf-8')).decode('utf-8')
    ...
    def get_ai_camera_sample_base64(self):
        sample_path = path.join(path.abspath(path.dirname(__file__)), 'assets',
                                'ai_camera_sample.jpg')
    ...
    def complete_ai_camera_task(self):
        ...
        recognize_payload = json.dumps(
            ...
            ensure_ascii=False,
            separators=(',', ':'))
        recognize_data = self.request_json(
        ...
        if not file_id:
            ...
            return False
        task_id = str(
            recognize_result.get('taskId') or int(time.time() * 1000))
        file_name = f'{int(task_id) + 1}.jpeg' if task_id.isdigit(
        ) else f'{task_id}.jpeg'
        input_time = datetime.now(timezone(
            timedelta(hours=8))).isoformat(timespec='milliseconds')
        chat_payload = json.dumps(
            ...
            ensure_ascii=False,
            separators=(',', ':'))
        chat_response = self.send_request(
        ...
        if chat_data and (chat_data.get('success')
                          or chat_data.get('code') == '0000'):
            return True
        if chat_data:
            self.log(
                f"AI相机对话失败: {chat_data.get('message') or chat_data.get('msg', '未知错误')}"
            )
            return False
    ...
    def complete_travel_guide_ai_task(self):
        ...
        if chat_data:
            self.log(
                f"出行攻略问AI失败: {chat_data.get('message') or chat_data.get('msg', '未知错误')}"
            )
            return False
    ...
    def complete_monthly_upload_task(self, task):
        ...
        for attempt in range(3):
            ...
            if remaining == 0:
                return True
            self.log(
                f'-{"开始" if attempt == 0 else "继续"}补上传进度: 当前{current_process}/{target_count}，还需{remaining}次'
            )
            success = 0
            ...
            if refreshed_task.get(
                    'state') == 'FINISH' or refreshed_process >= target_count:
                return True
    ...
    def complete_mayday_memory_task(self, task):
        ...
        if remaining == 0:
            return True
        self.log(
            f'-开始补五一回忆上传: 当前{current_process}/{target_count}，还需{remaining}张')
        success = 0
        for _ in range(remaining):
    ...
    def handle_cloud_v2_task(self, group, task):
        ...
        if task_id == 522:
            ...
            self.log(
                f'-需手动完成: {task_name}{self.get_task_progress(refreshed_task)}')
            return
        if task_id == 434:
            self.log(f'-去完成: {task_name}')
            ...
            if refreshed_task:
                ...
                if refreshed_task.get('state') == 'FINISH':
                    ...
                    return
                self.log(
                    f'-分享成功: {refreshed_name}{self.get_task_progress(refreshed_task)}'
                )
                return
        ...
        if task_id == 585:
            self.log(f'-去完成: {task_name}')
            if 'click' in self.get_task_step_types(task) and int(
                    task.get('currstep') or 0) == 0:
                click_data = self.click_task(task_id)
                if not click_data or click_data.get('code') != 0:
                    msg = click_data.get('msg',
                                         '未知错误') if click_data else '接口无响应'
            ...
            if self.complete_ai_camera_task():
                ...
                if refreshed_task.get('state') == 'FINISH':
                    ...
                    self.log(
                        f'-AI相机已体验: {refreshed_name}{self.get_task_progress(refreshed_task)}'
                    )
                return
            refreshed_task = self.query_cloud_task(task_id, group) or task
            self.log(
        ...
        if task_id == 587:
            ...
            if self.complete_mayday_memory_task(task):
                ...
                if refreshed_task.get('state') == 'FINISH':
                    ...
                    self.log(
                        f'-五一回忆已上传: {refreshed_name}{self.get_task_progress(refreshed_task)}'
                    )
                return
            refreshed_task = self.query_cloud_task(task_id, group) or task
            self.log(
        ...
        if task_id == 588:
            ...
            if self.complete_travel_guide_ai_task():
                ...
                if refreshed_task.get('state') == 'FINISH':
                    ...
                    self.log(
                        f'-出行攻略已生成: {refreshed_name}{self.get_task_progress(refreshed_task)}'
                    )
                return
            refreshed_task = self.query_cloud_task(task_id, group) or task
            self.log(
        ...
        if task_id == 589:
            ...
            if self.complete_holiday_nine_grid_task():
                ...
                if refreshed_task.get('state') == 'FINISH':
                    ...
                    self.log(
                        f'-假期九宫格已生成: {refreshed_name}{self.get_task_progress(refreshed_task)}'
                    )
                return
            refreshed_task = self.query_cloud_task(task_id, group) or task
            self.log(
    ...
    def sso(self):
        ...
        sso_data = self.request_json(sso_url,
                                     headers=sso_headers,
                                     data=sso_payload,
                                     method='POST')
        if not sso_data:
    ...
    def jwt(self):
        ...
        if token is not None:
            ...
            jwt_data = self.request_json(jwt_url,
    ...
    def signin_status(self):
        ...
        check_data = self.request_market_json(check_url,
                                              params={'client': 'app'})
        if not check_data:
        ...
        if self.get_today_sign_state(check_data.get('result', {})):
            self.log('✅已签到')
            return
        signin_data = self.request_market_json(
            f'{self.market_base_url}/market/signin/page/startSignIn',
            params={'client': 'app'})
        if not signin_data:
            self.log('签到失败: 接口无响应')
            return
        if signin_data.get('code') == 0 and self.get_today_sign_state(
                signin_data.get('result', {})):
            self.log('✅签到成功')
            return
        latest_data = self.request_market_json(check_url,
                                               params={'client': 'app'})
        if latest_data and latest_data.get(
                'code') == 0 and self.get_today_sign_state(
                    latest_data.get('result', {})):
            self.log('✅签到成功')
            return
        self.log(f"签到失败: {signin_data.get('msg', '未知错误')}")
        if signin_data.get('code') in (
                614, 615) and not get_env_device_id() and not get_device_id(
                    self.account):
    ...
    def refresh_notetoken(self):
        ...
        note_headers = {
            'X-Tingyun-Id':
            'p35OnrDoP8k;c=2;r=1122634489;u=43ee994e8c3a6057970124db00b2442c::8B3D3F05462B6E4C',
            'Charset': 'UTF-8',
            'Connection': 'Keep-Alive',
            'User-Agent': 'mobile',
            'APP_CP': 'ios',
        ...
        try:
            response = self.send_request(note_url,
        ...
        if app_type == 'cloud_app':
            if task_type == 'day':
                if task_id == 106:
                    ...
                    return_data = self.send_request(url=note_url,
                                                    headers=headers,
                                                    data=payload,
                                                    method='POST').json()
                    if return_data is None:
                        return print('出错了')
                    self.notebook_id = return_data['notebooks'][0][
                        'notebookId']
                    print('开始创建笔记')
    ...
    def create_note(self, headers):
        ...
        create_note_data = self.send_request(note_url,
                                             headers=headers,
                                             data=payload,
                                             method="POST")
    ...
    def shake(self):
        ...
        try:
            for _ in range(self.click_num):
                return_data = self.send_request(url=url,
                                                cookies=self.cookies,
                                                headers=self.jwtHeaders,
                                                method='POST').json()
                time.sleep(1)
                shake_prize_config = return_data["result"].get(
                    "shakePrizeconfig")
    ...
    def surplus_num(self):
        ...
        draw_info_data = self.send_request(draw_info_url,
                                           headers=self.jwtHeaders).json()
        ...
        if draw_info_data.get('msg') == 'success':
            ...
            if remain_num > 50 - self.draw:
                for _ in range(self.draw):
                    self.sleep()
                    draw_data = self.send_request(
                        url=draw_url, headers=self.jwtHeaders).json()
    ...
    def cloud_game(self):
        ...
        game_info_data = self.send_request(game_info_url,
                                           headers=self.jwtHeaders,
                                           cookies=self.cookies).json()
        if game_info_data and game_info_data.get('code', -1) == 0:
            currnum = game_info_data.get('result', {}).get('info',
                                                           {}).get('curr', 0)
            count = game_info_data.get('result',
                                       {}).get('history',
                                               {}).get('0',
                                                       {}).get('count', '')
            rank = game_info_data.get('result',
                                      {}).get('history',
                                              {}).get('0', {}).get('rank', '')
            ...
            for _ in range(currnum):
                self.send_request(bigin_url,
                                  headers=self.jwtHeaders,
                                  cookies=self.cookies).json()
                print('-开始游戏,等待10-15秒完成游戏')
                time.sleep(random.randint(10, 15))
                end_data = self.send_request(end_url,
                                             headers=self.jwtHeaders,
                                             cookies=self.cookies).json()
    ...
    def receive(self):
        ...
        info_data = self.request_market_json(
            f'{self.market_base_url}/market/signin/page/infoV3',
            params={'client': 'app'})
        if not info_data:
        ...
        if pending_amount:
            ...
            receive_data = self.request_json(
                f'{self.market_base_url}/market/signin/page/receiveV2',
                params={'client': 'app'},
            ...
            if not receive_data:
                ...
                self.log(
                    f'-领取云朵:{receive_result.get("receive", pending_amount)}云朵')
                total_amount = receive_result.get('total', total_amount)
            else:
                latest_info_data = self.request_market_json(
                    f'{self.market_base_url}/market/signin/page/infoV3',
                    params={'client': 'app'})
                latest_result = latest_info_data.get(
                    'result', {}) if latest_info_data and latest_info_data.get(
                        'code') == 0 else {}
                latest_pending = latest_result.get('toReceive', pending_amount)
                latest_total = latest_result.get('total', total_amount)
                pending_delta = pending_amount - latest_pending if isinstance(
                    pending_amount, int) and isinstance(latest_pending,
                                                        int) else 0
                total_delta = latest_total - total_amount if isinstance(
                    total_amount, int) and isinstance(latest_total, int) else 0
                claimed_amount = total_delta or pending_delta or (
                    pending_amount if latest_pending == 0 else 0)
                if claimed_amount > 0:
                    self.log(f'-领取云朵:{claimed_amount}云朵')
                    total_amount = latest_total
                else:
                    self.log(f"领取云朵失败: {receive_data.get('msg', '未知错误')}")
                    if receive_data.get('code') in (
                            614, 615
                    ) and not get_env_device_id() and not get_device_id(
                            self.account):
                        self.log(
                            '-当前账号在 ydyp_device_ids.json 中尚未填写 deviceId，请抓包后手动补充'
                        )
                    self.log(f'-当前待领取:{pending_amount}云朵')
        else:
            self.log('-当前待领取:0云朵')
        self.sleep()
        prize_data = self.request_json(
            prize_url, headers=self.jwtHeaders, cookies=self.cookies) or {}
    ...
    def backup_cloud(self):
        ...
        backup_data = self.send_request(backup_url,
                                        headers=self.jwtHeaders).json()
        ...
        if state == -1:
            ...
            cur_data = self.send_request(cur_url,
                                         headers=self.jwtHeaders).json()
        ...
        expend_data = self.send_request(expend_url,
                                        headers=self.jwtHeaders,
                                        cookies=self.cookies).json()
        curMonthBackup = expend_data.get('result',
                                         {}).get('curMonthBackup', '')
        preMonthBackup = expend_data.get('result',
                                         {}).get('preMonthBackup', '')
        curMonthBackupTaskAccept = expend_data.get('result', {}).get(
            'curMonthBackupTaskAccept', '')
        nextMonthTaskRecordCount = expend_data.get('result', {}).get(
            'nextMonthTaskRecordCount', '')
        ...
        if preMonthBackup:
            if curMonthBackupTaskAccept:
                ...
                receive_data = self.send_request(receive_url,
                                                 headers=self.jwtHeaders,
                                                 cookies=self.cookies).json()
                if receive_data.get("code") != 0:
                    self.log(f'-领取失败:{receive_data.get("msg")}')
                else:
                    cloudCount = receive_data.get('result',
                                                  {}).get('cloudCount', '')
    ...
    def complete_notice_task(self, task_name):
        ...
        if first_status != 3:
            reward1_data = self.request_json(reward_url,
                                             headers=self.jwtHeaders,
                                             data={"type": 1},
                                             method='POST')
        ...
        if second_status == 2:
            reward2_data = self.request_json(reward_url,
                                             headers=self.jwtHeaders,
                                             data={"type": 2},
                                             method='POST')
...
if __name__ == "__main__":
    ...
    msg = msg.replace('-', ' ').replace('.', ' ').replace('!', '！').replace(
        '(', '（').replace(')', '）')
    msg = msg.replace('_', ' ').replace('=', ' ').replace('~', ' ').replace(
        '{', ' ').replace('}', ' ').replace('|', ' ')
